...
class RuleBasedParser:
    ...
    def _compile_patterns(self) -> Dict:
        ...
        return {
            ...
            'top_products': re.compile(r'top\s+(\d+)?\s*products?', re.I),
    ...
    def parse(self, query: str) -> ParsedQuery:
        ...
        # Revenue analysis
        if self.patterns['top_products_revenue'].search(query):
            match = self.patterns['top_products_revenue'].search(query)
            ...
            return ParsedQuery(
                ...
                explanation=f"Top {n} products by total revenue",
                confidence=0.95
            )
...
class LLMParser:
    ...
    def _initialize_model(self):
        try:
            ...
            logger.warning("Transformers not available - using rule-based parsing only")
...
Available operations: group_and_aggregate, filter_data, sort_data, pivot_data, preview
...
class QueryParser:
    ...
    def __init__(self, columns: List[str]):
        self.rule_parser = RuleBasedParser(columns)
